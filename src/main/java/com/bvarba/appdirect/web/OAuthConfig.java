...
import com.bvarba.appdirect.web.controller.AppDirectISVController;

@Configuration
public class OAuthConfig extends WebSecurityConfigurerAdapter {

  private static Logger logger = Logger.getLogger(OAuthConfig.class);
  ...
  public void configure(HttpSecurity http) throws Exception {
    logger.debug("Configure OAuth security for AppDirect API");

    // only validate signit
    OneLeggedOAuthFilter filter = new OneLeggedOAuthFilter();
    filter.setFilterProcessesUrl(AppDirectISVController.ENDPOINTS_CONSUMER_PATH);
    ...
    // setup
    http
          .antMatcher(AppDirectISVController.ENDPOINTS_CONSUMER_PATH)
          .csrf().disable()
